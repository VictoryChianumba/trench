...
- [x] Command mode `:` — `Mode::Command` + `cmd_buf`; `crates/block-reader/src/commands.rs` parser + dispatcher. v1 ships ~17 commands: `:q`/`:quit`, `:<N>` (goto line), `:goto <N|text>`, `:abstract`, `:references`, `:set theme=<id>` (with `trench` for sync), `:marks`, `:delmarks <letter>`, `:highlights`, `:about`, `:url`, `:cite`/`:bibtex`, `:open`, `:back`, `:help`, `:toc`, `:placement`. Theme persistence at `~/.config/trench/block_reader.json` follows trench's theme by default; override syncs back to `trench` via `:set theme=trench`. Deferred: `:reload`, `:next`/`:prev`, tab completion, history.
