...
use crate::ui::ui_functions::centered_rect;

const FOOTER_TEXT: &str = r"<Space>: Toggle Selected | Enter or <Ctrl-m>: Confirm | Esc, q or <Ctrl-c>: Cancel";
...
impl TagsPopup {
  pub fn new(tags_text: &str, mut tags: Vec<String>) -> Self {
    ...
    let unsaved_tags: Vec<String> =
      existing_tags.iter().filter(|tag| !tags.contains(tag)).cloned().collect();

    unsaved_tags.into_iter().rev().for_each(|tag| tags.insert(0, tag));

    let selected_tags = BTreeSet::from_iter(existing_tags);

    let mut tags_popup = Self { state, tags, selected_tags };
  ...
  pub fn render_widget(&mut self, frame: &mut Frame, area: Rect) {
    ...
    let chunks = Layout::default()
      ...
      .vertical_margin(1)
      .constraints(
        [Constraint::Min(3), Constraint::Length(footer_height)].as_ref(),
      )
      .split(area);
  ...
  pub fn handle_input(&mut self, key: KeyEvent) -> TagsPopupReturn {
...
mod tests {
  ...
  fn new_prepends_unsaved_tags() {
    ...
    assert_eq!(
      popup.tags,
      vec![String::from("fresh"), String::from("saved"), String::from("old")]
    );
  ...
  fn navigation_wraps_both_ways() {
    let mut popup =
      TagsPopup::new("one", vec![String::from("one"), String::from("two")]);
  ...
  fn toggle_adds_and_removes() {
    let mut popup =
      TagsPopup::new("", vec![String::from("one"), String::from("two")]);
  ...
  fn confirm_keeps_list_order() {
    let mut popup =
      TagsPopup::new("beta", vec![String::from("alpha"), String::from("beta")]);
