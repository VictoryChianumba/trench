...
impl ExportPopup<'_> {
    pub fn create_note_content(note: &Note, default_path: Option<&Path>) -> anyhow::Result<Self> {
    ...
    pub fn create_multi_select(count: usize, default_path: Option<&Path>) -> anyhow::Result<Self> {
        let mut path = if let Some(p) = default_path {
            p.to_path_buf()
        } else {
            env::current_dir()?
    ...
    pub fn render_widget(&mut self, frame: &mut Frame, area: Rect) {
        ...
        if self.path_err_msg.is_empty() {
            ...
            let cursor_style = Style::default().fg(Color::Yellow).bg(Color::Yellow);
