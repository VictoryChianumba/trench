...
impl Default for Filter {
  fn default() -> Self {
    Filter { relation: CriteriaRelation::And, criteria: Vec::new() }
  }
}
...
impl Filter {
  ...
  pub fn check_note(&self, note: &Note) -> bool {
    match self.relation {
      CriteriaRelation::And => {
        self.criteria.par_iter().all(|cr| cr.check_note(note))
      }
      CriteriaRelation::Or => {
        self.criteria.par_iter().any(|cr| cr.check_note(note))
      }
    }
  }
}
