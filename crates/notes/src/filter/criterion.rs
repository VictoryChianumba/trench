...
impl FilterCriterion {
    ...
    pub fn check_note(&self, note: &Note) -> bool {
        match self {
            FilterCriterion::Tag(TagFilterOption::Tag(tag)) => note.tags.contains(tag),
...
mod tests {
    ...
    fn tag_checks_match_exactly() {
        ...
        assert!(
            FilterCriterion::Tag(TagFilterOption::Tag(String::from("rust"))).check_note(&note)
        );
        assert!(
            !FilterCriterion::Tag(TagFilterOption::Tag(String::from("Rust"))).check_note(&note)
        );
    ...
    fn no_tags_requires_empty_list() {
        ...
        assert!(!FilterCriterion::Tag(TagFilterOption::NoTags).check_note(&tagged_note));
    }
    ...
    fn content_search_uses_smart_case() {
        ...
        assert!(FilterCriterion::Content(String::from("content")).check_note(&note));
        assert!(FilterCriterion::Content(String::from("Mixed")).check_note(&note));
        assert!(
            !FilterCriterion::Content(String::from("mixed")).check_note(&Note {
                article_id: "2".into(),
