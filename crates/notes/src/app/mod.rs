...
use crate::{
    ...
    keymap::{
        Input, UICommand,
        get_editor_mode_keymaps, get_entries_list_keymaps,
        get_global_keymaps, get_multi_select_keymaps,
    },
    ...
    ui::{
        ...
        msg_box::{MsgBox, MsgBoxActions, MsgBoxInputResult, MsgBoxResult, MsgBoxType},
...
impl App {
    ...
    pub fn apply_initial_focus(&mut self) {
        ...
        if let Some(pos) = pos {
            ...
            self.active_popup = ActivePopup::CreateNote(Box::new(NotePopup::from_note(&stub)));
        }
    }
    ...
    pub fn handle_input(&mut self, key: KeyEvent) -> HandleInputReturn {
        ...
        match self.notes_state {
            ...
            NotesState::List
            | NotesState::Preview
            | NotesState::PreviewScroll => self.handle_list_input(key),
        }
    }
    ...
    fn handle_editor_action(&mut self, action: NoteEditorAction) -> HandleInputReturn {
    ...
    fn exec_command(&mut self, cmd: UICommand) -> HandleInputReturn {
        match cmd {
            ...
            UICommand::CycleFocusedControlForward | UICommand::CycleFocusedControlBack => {
            ...
            UICommand::MulSelSelectAll => {
                self.entries_list.selected_notes = self
                    .get_active_notes()
    ...
    fn show_err_msg(&mut self, msg: String) {
        self.active_popup =
            ActivePopup::MsgBox(MsgBox::new(MsgBoxType::Error(msg), MsgBoxActions::Ok));
    }
    ...
    pub fn draw(&mut self, frame: &mut Frame, area: Rect) {
        // Outer vertical split: content + one-line footer.
        ...
        match self.notes_state {
            NotesState::Editor => {
                if self.editor.is_insert_mode() {
                    "<Ctrl-s>: Save  Esc: Normal mode".into()
                } else if self.editor.is_visual_mode() {
                    "<Ctrl-s>: Save  Esc: Normal mode".into()
                } else {
                    "<Ctrl-s>: Save  Esc: Back to list  i: Insert".into()
                }
            }
            NotesState::List => {
                "j/k: Navigate  Enter: Edit  n: New  d: Del  f: Filter  o: Sort  /: Find  q: Quit"
                    .into()
            }
            NotesState::Preview => {
                "j/k: Navigate  Tab: Focus preview  Enter: Edit  Esc: Close  q: Quit".into()
            }
            NotesState::PreviewScroll => {
                "j/k: Scroll preview  Tab: Back to list  Enter: Edit  Esc: Close  q: Quit".into()
            }
        }
    }

    fn draw_preview(&self, frame: &mut Frame, area: Rect, note: &Note) {
        let focused = self.notes_state == NotesState::PreviewScroll;
        let border_style = if focused {
            Style::default().fg(Color::Yellow)
        } else {
            Style::default().fg(Color::DarkGray)
        };
        let title = format!(" {} ", note.article_title);
    ...
    fn sort_notes(&mut self) {
        self.notes
            .sort_by(|a, b| self.sorter.sort(a, b));
    }
    ...
    pub fn open_create_note_popup(&mut self) {
        self.active_popup = ActivePopup::CreateNote(Box::new(NotePopup::new_note()));
    }

    pub fn open_edit_note_popup(&mut self) {
        if let Some(note) = self.get_current_note() {
            self.active_popup = ActivePopup::EditNote(Box::new(NotePopup::from_note(note)));
        }
    }
    ...
    pub fn open_filter_popup(&mut self) {
        ...
        self.active_popup = ActivePopup::Filter(Box::new(FilterPopup::new(tags, existing)));
    }
