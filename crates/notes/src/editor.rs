...
impl<'a> NoteEditor<'a> {
    ...
    pub fn refresh_has_unsaved(&mut self) {
        self.has_unsaved = self.is_dirty && self.get_content() != self.original_content;
    }
    ...
    fn handle_input_visual_only(&mut self, key: KeyEvent) -> anyhow::Result<bool> {
    ...
    pub fn render_widget(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let border_color = match (self.mode, self.is_active) {
            (EditorMode::Insert, _) => Color::Yellow,
            (EditorMode::Visual, _) => Color::Green,
            (EditorMode::Normal, true) => Color::White,
            (EditorMode::Normal, false) => Color::DarkGray,
        };
        ...
        let cursor_style = if self.is_active {
            match self.mode {
                ...
                EditorMode::Visual => Style::default().add_modifier(Modifier::UNDERLINED),
            }
        } else {
            Style::reset()
        };
    ...
    pub fn render_vertical_scrollbar(&mut self, frame: &mut Frame, area: Rect) {
        ...
        if lines_count as u16 <= area.height.saturating_sub(2) {
            return;
        }
    ...
    pub fn render_horizontal_scrollbar(&mut self, frame: &mut Frame, area: Rect) {
        ...
        if max_width as u16 <= area.width.saturating_sub(2) {
            return;
        }
    ...
    pub(crate) fn exec_os_clipboard(&mut self, operation: ClipboardOperation) -> anyhow::Result<()> {
